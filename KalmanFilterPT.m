...
z_vec = [z.L(end)/100*p.height_PT; z.F_in(end)]; % Redefine the measurement matrix with latest measurements

x0 = [x.L(end); x.F_in(end); x.h_PT(end); x.P(end)];
%x0 = [x.L(end); x.F_in(end); x.P(end)];

[~,x_integrate] = ode45(@(t,x) KFPreCoolingTowerODEs(s, p, x, u, t, output), t, x0);


% Define the state estimates vs measurement matrices
x_L = x_integrate(end,1)/100*p.height_PT; % Convert previous state estimate to height
...
x.L(end+1) = x_K(1)/p.height_PT*100; % Kalman estimate for PT Level
